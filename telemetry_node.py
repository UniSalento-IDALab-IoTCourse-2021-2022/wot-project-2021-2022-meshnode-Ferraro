...
class MyTelemetry(Model):
	...
	def set_publication(self, period):
		...
		self.state = (20 + random.randint(0, 11))

		self.pub_timer.start(period/1000, self.publish)

	def publish(self):
		print('Publish')
		data = struct.pack('>Hi', 0xFFFE, int(self.state))
		self.send_publication(data)
...
def main():
	...
	second_ele.add_model(MyTelemetry(0x0003))
